...
public class PowerZero extends Scenario {
	public TStatement[] getScenario() {
		return new TStatement[] {
			new TVarDecl(BT.S,"Power","p"),
			new TLocalAssign(BT.S,"p",new TNew(BT.Bot,"Power",new TExp[] { new TConstant(0)}))		,
			new TVarDecl(BT.D,"int","arg"),
			new TLocalAssign(BT.D,"arg",new DynamicValue("int","b"))		,
			new TReturn(BT.D,new TCall(BT.Bot,null,"p","raise",new TExp[] { new TVarRef(BT.Bot,"arg")}))
		};
	}
	public Program getProgram() {
		return new Program("PowerZero",
			this.getScenario(),
new TType[] {
